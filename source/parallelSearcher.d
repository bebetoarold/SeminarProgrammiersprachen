                ...
                foreach (kv; parallel(arrayMatchedLinesToLineNumber.byPair, 100))
                {
                    ...
                    if (before_context)
                    {
                        for (int i = 1; i <= before_context; i++)
                        {
                            if (kv.key - i > 0)
                            {
                                writeln("Before Context of ->", red ~ entry ~ reset, ":" ~ green, kv.key - i,
                                        reset ~ ":", highlightMatch(keeperArray[to!int(kv.key - i)],
                                            regex, color));
                            }
                        }
                    }
                    writeln(red ~ entry ~ reset, ":" ~ green, kv.key, reset ~ ":",
                            highlightMatch(kv.value, regex, color));
                    ...
                    if (after_context)
                    {
                        for (int i = 1; i <= after_context; i++)
                        {
                            ...
                            {
                                writeln("After Context of ->", red ~ entry ~ reset, ":" ~ green, kv.key + i,
                                        reset ~ ":", highlightMatch(keeperArray[to!int(kv.key + i)],
                                            regex, color));
                            }
                        }
                ...
                foreach (kv; parallel(arrayMatchedLinesToLineNumber.byPair, 100))
                {
                    ...
                    if (before_context)
                    {
                        for (int i = 1; i <= before_context; i++)
                        {
                            if (kv.key - i > 0)
                            {
                                writeln("Before Context:-> " ~ green, kv.key - i, reset ~ ":",
                                        highlightMatch(keeperArray[to!int(kv.key - i)],
                                            regex, color));
                            }
                        }
                    }

                    writeln(green ~ "", kv.key, "" ~ reset, ":",
                            highlightMatch(kv.value, regex, color));
                    ...
                    if (after_context)
                    {
                        for (int i = 1; i <= after_context; i++)
                        {
                            if (kv.key + i <= keeperArray.length)
                            {
                                writeln("After Context:-> " ~ green, kv.key + i, reset ~ ":",
                                        highlightMatch(keeperArray[to!int(kv.key + i)],
                                            regex, color));
                            }
                        }
...
void main(string[] args)
{
    ...
    else
    {
        ...
        */
        ...
        foreach (path; parallel(pathList))
        {
