...
import std.datetime; // for Clock
import std.utf; // for UTFException

/** @brief: SEQUENTIAL SEARCHER PROGRAM
 * @brief: SEQUENTIAL SEARCHER PROGRAM
...
void readFile(File file, string entry, Regex!char regex, int after_context,
        int before_context, int context, bool no_heading, bool color)
{
    ...
    //checking for encoding and others
    try
    {
        ...
        if (numberMatchedPatternPerFile != 0)
        {
            ...
            if (no_heading)
            {
                ...
                writeln(printHeading(color), entry, "-------------(",
                        numberMatchedPatternPerFile, ")");
                writeln(reset);
                ...
                foreach (key, value; arrayMatchedLinesToLineNumber)
                {
                    ...
                    if (before_context)
                    {
                        for (int i = 1; i <= before_context; i++)
                        {
                            if (key - i > 0)
                            {
                                writeln("Before Context of ->", red ~ entry ~ reset, ":" ~ green, key - i,
                                        reset ~ ":", highlightMatch(keeperArray[key - i],
                                            regex, color));
                            }
                        }
                    }
                    writeln(red ~ entry ~ reset, ":" ~ green, key, reset ~ ":",
                            highlightMatch(value, regex, color));
                    ...
                    if (after_context)
                    {
                        for (int i = 1; i <= after_context; i++)
                        {
                            ...
                            {
                                writeln("After Context of ->", red ~ entry ~ reset, ":" ~ green, key + i,
                                        reset ~ ":", highlightMatch(keeperArray[key + i],
                                            regex, color));
                            }
                        }
            ...
            {
                writeln(red ~ printHeading(color), entry, "-------------(",
                        numberMatchedPatternPerFile, ")");
                writeln(reset);
                ...
                foreach (key, value; arrayMatchedLinesToLineNumber)
                {
                    ...
                    if (before_context)
                    {
                        for (int i = 1; i <= before_context; i++)
                        {
                            if (key - i > 0)
                            {
                                writeln("Before Context:-> " ~ green, key - i, reset ~ ":",
                                        highlightMatch(keeperArray[key - i], regex, color));
                            }
                        }
                    }
                    ...
                    if (after_context)
                    {
                        for (int i = 1; i <= after_context; i++)
                        {
                            if (key + i <= keeperArray.length)
                            {
                                writeln("After Context:-> " ~ green, key + i, reset ~ ":",
                                        highlightMatch(keeperArray[key + i], regex, color));
                            }
                        }
                    }
...
void main(string[] args)
{
    ...
    else
    {
        for (int i = 0; i < args.length; i++)
        {
            ...
            {
                writeln("usage(SEQUENTIAL): searcher [OPTIONS] PATTERN [PATH ...]");
                writeln(
                        "-A,--after-context <arg> prints the given number of following lines for each match");
                writeln(
                        "-B,--before-context <arg> prints the given number of preceding lines for each match");
                            ...
                            if (isFile(entry))
                            {
                                ...
                                if (hidden)
                                {
                                    if (baseName(entry).startsWith("."))
                                    {
                                        ...
                                        readFile(File(entry, "r"), entry, regex, after_context,
                                                before_context, context, no_heading, color);
                    ...
                    else if (isFile(path))
                    {
                        ...
                        if (hidden)
                        {
                            if (baseName(path).startsWith("."))
                            {
                                ...
                                readFile(File(path, "r"), path, regex, after_context,
                                        before_context, context, no_heading, color);
